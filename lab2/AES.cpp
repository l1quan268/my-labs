...
std::vector<std::vector<uint8_t>> AES::key_expansion_256()
{
    ...
    for (int i = 8; i < 60; i++) // 60 words for AES-256
    {
        ...
        if (i % key_words == 0)
        {
            ...
            for (std::size_t j = 0; j < 4; j++)
            {
                temp[j] ^= AES::RCON[(i - key_words) / key_words][j];
            }
        }
        else if (i % key_words == 4)  // THÊM ĐIỀU KIỆN NÀY CHO AES-256
        {
            temp = sub_word(temp);  // Chỉ SubWord, không RotWord và RCON
        }
        std::vector<uint8_t> new_round_key;
        for (std::size_t j = 0; j < 4; j++)
        {
...
void AES::add_round_key(std::vector<std::vector<uint8_t>> &state, int round_number)
{
    for (int i = 0; i < 4; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            state[i][j] ^= round_keys[round_number * 4 + j][i];  // SỬA: đổi thứ tự index
        }
    }
}
...
std::vector<uint8_t> AES::encrypt(const std::vector<uint8_t> &data)
{
    ...
    // SỬA: Convert data into state matrix đúng cách
    std::vector<std::vector<uint8_t>> state(4, std::vector<uint8_t>(4, 0));
    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t j = 0; j < 4; ++j)
        {
            state[i][j] = data[i + 4 * j];  // Đúng cách chuyển đổi
        }
    }
    ...
    // SỬA: Convert state matrix back to bytes đúng cách
    std::vector<uint8_t> encrypted_data(16);
    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t j = 0; j < 4; ++j)
        {
            encrypted_data[i + 4 * j] = state[i][j];  // Đúng cách chuyển đổi
        }
    }
...
std::vector<uint8_t> AES::decrypt(const std::vector<uint8_t> &ciphertext)
{
    ...
    // SỬA: Convert ciphertext into state matrix đúng cách
    std::vector<std::vector<uint8_t>> state(4, std::vector<uint8_t>(4, 0));
    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t j = 0; j < 4; ++j)
        {
            state[i][j] = ciphertext[i + 4 * j];  // Đúng cách chuyển đổi
        }
    }
    ...
    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t j = 0; j < 4; ++j)
        {
            decrypted_data[i + 4 * j] = state[i][j];  // Đúng cách chuyển đổi
        }
    }
